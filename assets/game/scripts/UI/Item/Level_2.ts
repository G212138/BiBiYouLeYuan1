...
export default class Level_2 extends cc.Component {
    ...
    onLoad() {
        ...
        ListenerManager.on(EventType.DRAG_OPTION_START, this.updateOptions, this);
    }
    onDestroy() {
        ...
        ListenerManager.off(EventType.DRAG_OPTION_START, this.updateOptions, this);
    }
    ...
    private handleShowCircle(needAnim: boolean = false) {
        ...
        for (let i = 0; i < this.diyidui.childrenCount; i++) {
            if (needAnim) {
                this.diyidui.children[i].getChildByName("heibai").active = true;
                this.diyidui.children[i].getChildByName("heibai").getComponent(sp.Skeleton).timeScale = 2;
                Tools.playSpine(this.diyidui.children[i].getChildByName("heibai").getComponent(sp.Skeleton), "eft_smoke", false, () => {
                    this.diyidui.children[i].getChildByName("heibai").active = false;
                });
            } else {
                this.diyidui.children[i].getChildByName("heibai").active = false;
            }
